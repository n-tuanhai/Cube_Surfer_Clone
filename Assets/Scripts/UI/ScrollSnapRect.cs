...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    void Start()
    {
        ...
        if (nextToggle)
            nextToggle.onValueChanged.AddListener(delegate(bool arg0)
            {
                if (arg0 && _currentPage != 1)
                    NextScreen();
            });

        if (prevToggle)
            prevToggle.onValueChanged.AddListener(delegate(bool arg0)
            {
                if (arg0 && _currentPage != 0)
                    PreviousScreen();
            });
    }
    ...
    public void UpdateToggleValue()
    {
        if (_currentPage == 0)
        {
            prevToggle.isOn = true;
            toggleGroup.NotifyToggleOn(prevToggle);
        }
        else
        {
            nextToggle.isOn = true;
            toggleGroup.NotifyToggleOn(nextToggle);
        }
    }
